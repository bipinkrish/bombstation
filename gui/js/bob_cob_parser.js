/**
 * gui/js/bob_cob_parser.js — Ballistica .bob and .cob Binary Mesh Decoder
 * 
 * Directly parses binary Ballistica 3D assets in the browser:
...
/**
 * Parses an ArrayBuffer containing a .bob visual model and generates a Three.js BufferGeometry.
 */
export function parseBobToArray(arrayBuffer) {
  ...
  if (availableBytes >= vertexCount * stride) {
    ...
    // 12-byte position only fallback
    for (let i = 0; i < vertexCount; i++) {
      positions[i * 3 + 0] = view.getFloat32(offset + 0, true);
      positions[i * 3 + 1] = view.getFloat32(offset + 4, true);
      positions[i * 3 + 2] = view.getFloat32(offset + 8, true);
...
/**
 * Parses an ArrayBuffer containing a .cob collision hull and generates position/index arrays.
 */
export function parseCobToArray(arrayBuffer) {
  const view = new DataView(arrayBuffer);
...
/**
 * Builds a Three.js BufferGeometry from parsed .bob or .cob data.
 */
export function buildThreeGeometry(parsed) {
